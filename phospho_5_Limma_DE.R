...
for (t in tissues)
{
  design <- read.csv(paste0('./phospho_DE/Cod_', t, '.csv'), row.names = 1, header = 1)
  exp_matrix <- read.csv(paste0('./phospho_DE/Exp_', t, '.csv'), row.names = 1, header = 1)
  ...
  for (c in c("acute11p", "acute8p", "chronic11p", "chronic8p", "precondition11p", "precondition8p", "precondition8p11p", "precondition8pacute8p"))
  {
    ...
    write.csv(result, paste0('./phospho_DE/', t, '/limma_', c, '.csv'))
  }
}
